...
module Helpers
  def self.generate_code_verifier = Clients::Twitter::Utils::PKCE.code_verifier

  def self.generate_code_challenge(code_verifier)
    Clients::Twitter::Utils::PKCE.code_challenge(code_verifier)
  end

  def self.generate_authorize_url(client_id: ENV['TWITTER_CLIENT_ID'], state:, code_challenge:)
